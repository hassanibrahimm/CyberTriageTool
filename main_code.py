...
class NUCESVideoTriage:
    ...
    def display_congratulations(self):
        ...
        congrats_label = tk.Label(
            self.root,
            text="Congratulations! Your video has been successfully analyzed.",
            font=("Courier", 20, "bold"),
            fg="green",
            bg="black",
        )
        congrats_label.place(relx=0.5, rely=0.3, anchor="center")
        ...
        metadata_button.place(relx=0.3, rely=0.5, anchor="center")
        ...
        faces_button.place(relx=0.7, rely=0.5, anchor="center")
    ...
    def display_faces(self):
        ...
        if self.face_images:
            ...
            self.face_panel = tk.Label(self.root, bg="black")
            self.face_panel.place(relx=0.5, rely=0.4, anchor="center", width=300, height=300)

            navigation_frame = tk.Frame(self.root, bg="black")
            navigation_frame.place(relx=0.5, rely=0.8, anchor="center")
    ...
    def next_face(self):
        if self.face_index < len(self.face_images) - 1:
            self.face_index += 1
            self.display_face_image()
    ...
    def display_face_image(self):
        if self.face_images:
            ...
            face_image = Image.open(face_image_path).resize((300, 300), Image.Resampling.LANCZOS)
            face_photo = ImageTk.PhotoImage(face_image)
            self.face_panel.config(image=face_photo)
            self.face_panel.image = face_photo
